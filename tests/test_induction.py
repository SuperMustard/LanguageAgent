...
def test_apply_mastery_updates_disambiguates_targets_with_colliding_db_ids():
    # 回归测试：word 和 pro_phrase 各自的表都从 id=1 自增，撞号是完全可能的。
    # 用列表位置对齐后，两个目标的更新互不干扰，即使 db id 相同。
    conn = db.connect(":memory:")
    w = _word(mastery=0)
    db.insert_words(conn, [w])
    p = _phrase(mastery=0)
    db.insert_pro_phrases(conn, [p])
    assert w.id == p.id == 1  # 前提：确实撞号了，不然这个回归测试没有意义
    ...
    apply_mastery_updates(
        conn, targets, {0: "used_correctly", 1: "used_incorrectly"}, "2026-08-28T00:00:00+00:00"
    )
